...
from lockin.styles import AppStyles as Styles
from lockin.validators import not_null
from lockin.exceptions import DuplicateServiceError
...
class LockIn(toga.App):
    ...
    def save_service(self, *args, **kwargs):
        ...
        if all([not_null(val) for val in inputs]):
            try:
                ...
                self.encrypt_popup.error_dialog(
                    "Error",
                    f"Service: '{self.service_name_input.value}' already exists.",
                )
                return

            if saved:
                self.encrypt_popup.confirm_dialog(
                    "Saved!", f"New service saved: {self.service_name_input.value}"
                )
        ...
        self.encrypt_popup.error_dialog(
            "Error!", f"Error saving service: {self.service_name_input.value}"
        )

    def fetch_service(self, *args, **kwargs):
        try:
            username, password = self.manager.fetch_service(
                self.selected_service, self.decryption_password.value
            )
        except TypeError:
            self.decrypt_popup.error_dialog(
                "Error!", f"Password '{self.decryption_password.value}' incorrect."
            )
            return

        self.decrypt_popup.info_dialog(
            "Success!", f"Username: {username}\nPassword: {password} "
        )
        self.decrypt_popup.close()

    def delete_service(self, *args, **kwargs):
        deleted = self.manager.delete_service(
            self.selected_service, self.delete_decryption_password.value
        )
        if deleted:
            self.delete_popup.info_dialog(
                "Success!", f"Service: '{self.selected_service}' deleted."
            )
            self.delete_popup.close()
            self.gen_service_table()
            return
        self.delete_popup.error_dialog(
            "Error",
            f"Password '{self.delete_decryption_password.value}' incorrect. Service not deleted.",
        )
    ...
    def encrypt_popup_handler(self, *args, **kwargs):
        ...
        self.service_name_input = toga.TextInput(
            placeholder="Service Name", style=Styles.ENCRYPT_TEXT_INPUT_STYLE
        )
        self.service_username_input = toga.TextInput(
            placeholder="Username", style=Styles.ENCRYPT_TEXT_INPUT_STYLE
        )
        self.service_password_input = toga.TextInput(
            placeholder="Password", style=Styles.ENCRYPT_TEXT_INPUT_STYLE
        )
        self.encryption_password = toga.TextInput(
            placeholder="Encryption Password", style=Styles.ENCRYPT_TEXT_INPUT_STYLE
        )
        self.save_button = toga.Button(
            "Save", style=Styles.SAVE_BUTTON_STYLE, on_press=self.save_service
        )
    ...
    def decrypt_popup_handler(self, *args, **kwargs):
        ...
        self.decrypt_label = toga.Label(
            f"Please enter decryption password.", style=Styles.DECRYPT_LABEL_STYLE
        )
        self.decryption_password = toga.TextInput(
            placeholder="Password", style=Styles.DECRYPT_TEXT_INPUT_STYLE
        )
        self.decrypt_button = toga.Button(
            "Decrypt", style=Styles.DECRYPT_BUTTON_STYLE, on_press=self.fetch_service
        )
    ...
    def delete_popup_handler(self, *args, **kwargs):
        ...
        self.delete_label = toga.Label(
            f"Please enter decryption password.", style=Styles.DECRYPT_LABEL_STYLE
        )
        self.delete_decryption_password = toga.TextInput(
            placeholder="Password", style=Styles.DECRYPT_TEXT_INPUT_STYLE
        )
        self.delete_button = toga.Button(
            "Delete", style=Styles.DECRYPT_BUTTON_STYLE, on_press=self.delete_service
        )
